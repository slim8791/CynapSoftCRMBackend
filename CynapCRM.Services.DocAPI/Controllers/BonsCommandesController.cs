...
namespace CynapCRM.Services.DocAPI.Controllers
{
    ...
    public class BonsCommandesController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllBonsCommande(
            ...
            [FromQuery] int pageSize = 20)
        {
            try
            {
                ...
                var result = await _bCService.GetAllBonsCommandeAsync(pageNumber, pageSize);
        ...
        [Authorize(Roles = "ADMIN,SUPERVISEUR,DELEGUE,PHARMACIEN,GROSSISTE,CLIENT")]
        public async Task<IActionResult> GetBonsCommandeByCommande(int idCommande)
        {
            try
            {
                if (idCommande <= 0)
                ...
                var list = await _bCService.GetBonsCommandeByCommandeAsync(idCommande);
                _response.Result = list;
                return Ok(_response);
            }
            catch (Exception ex)
        ...
        [HttpGet("by-date")]
        [Authorize(Roles = "ADMIN,SUPERVISEUR")]
        public async Task<IActionResult> GetBonsCommandeByDate(
            ...
            [FromQuery] DateTime endDate)
        {
            try
            {
                ...
                var result = await _bCService.GetBonsCommandeByDateAsync(startDate, endDate);
                _response.Result = result;
                return Ok(_response);
            }
            catch (Exception ex)
        ...
        [Authorize(Roles = "ADMIN,SUPERVISEUR")]
        public async Task<IActionResult> CreateOrUpdateBonCommande(
            [FromBody] BonCommandeDto bcDto)
        {
            try
            {
        ...
        public async Task<IActionResult> DeleteBonCommande(int id)
        {
            try
            {
                if (id <= 0)
                ...
                var result = await _bCService.DeleteBonCommandeAsync(id);
                if (!result)
                {
                    _response.IsSuccess = false;
                    _response.Message = "Bon de commande introuvable.";
                ...
                _response.Message = "Bon de commande supprimé avec succès.";
                return Ok(_response);
            }
            catch (Exception ex)
            {
