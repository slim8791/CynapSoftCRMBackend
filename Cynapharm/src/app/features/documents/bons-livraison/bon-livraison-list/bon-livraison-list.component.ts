...
export class BonLivraisonListComponent implements OnInit, OnDestroy {
  ...
  constructor(private svc: BonLivraisonService, private cdr: ChangeDetectorRef, private pdf: PdfService, private userSvc: UserService) {}
  ngOnInit() { this.load(); }
  ...
  download(doc: BonLivraisonDto): void {
    ...
    if (!url) {
      this.error = 'Aucun fichier Cloudinary disponible pour ce bon de livraison.';
      return;
    }
